...
export default function ManageClasses() {
  ...
  const userModality = storedUser.modalidade || "";

  const [treinos, setTreinos] = useState([]);
  const [loading, setLoading] = useState(true);
  const [query, setQuery] = useState("");
  const [filterModalidade, setFilterModalidade] = useState(
    userType === "personal trainer" ? userModality : ""
  );
  ...
  const [createWorkoutForm, setCreateWorkoutForm] = useState({
    ...
    inicio: "",
    fim: "",
    modalidade: userType === "personal trainer" ? userModality : "",
    segunda: "",
    terca: "",
    quarta: "",
    quinta: "",
  ...
  async function fetchTreinos() {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/treinos`);
      const data = await res.json();
      let arr = Array.isArray(data.treinos)
        ? data.treinos
        : Array.isArray(data)
        ? data
        : [];
    ...
    try {
      ...
      if (data.success && Array.isArray(data.checkins)) {
        ...
        data.checkins.forEach((c) => {
          ...
          if (!diaKey) return; // ignora se não estiver mapeado
                ...
                <>
                  ...
                  <div className="custom-select-wrapper">
                    <select
                      ...
                      disabled={userType === "personal trainer"} // bloqueia se for personal
                    >
                      ...
                      {MODALIDADE_OPTIONS.map((m) => (
                        <option
                          key={m}
                          value={userType === "personal trainer" ? userModality : m}
                        >
                          {userType === "personal trainer" ? userModality : m}
                        </option>
                      ))}
                    </select>
              ...
              {stepCreateWorkout === 2 && (
                <>
                  {[
                    ...
                    <div key={key}>
                      <label>Aula de {label}</label>
                      <input
                        required
                        value={createWorkoutForm[key]}
                        onChange={(e) =>
                          setCreateWorkoutForm({
                            ...createWorkoutForm,
                            [key]: e.target.value,
                        ...
                        placeholder={`Descrição da aula de ${label}`}
                      />
                    </div>
                  ))}
                  <div className="modal-actions">
                ...
                <>
                  ...
                  <div className="custom-select-wrapper">
                    <select
                      ...
                      disabled={userType === "personal trainer"}
                    >
                      <option value="" disabled hidden>
                        Selecione
                      </option>
